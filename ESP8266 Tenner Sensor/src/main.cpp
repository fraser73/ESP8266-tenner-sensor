...
#include <PubSubClient.h>       // MQTT client for ESP8266
...
//Declare Functions
void restartESP();
String macToStr(uint8_t*);
void mqttCallback(char *topic, byte *payload, int length);
void colourWipe(uint32_t, uint8_t);
...
//Define WiFi client and PubSubClient (MQTT client)
...
// Set the MQTT Server Definition
mqttClient.setServer (server,1883);   // mqttClient.setServer ("192.168.1.17",1883);
...
//  }
  ...
  if (mqttClient.connect((char*) clientName.c_str(),NULL,NULL)) {
    if (neoPixels) {
      if (diags) {
...
void loop() {
  // static int counter = 0; // commented out as I don't know what this was supposed to do and it's unused...
  ...
  if (temperatureLogging) {
    ...
    mqttClient.publish("temperatureTopic", "temperatureString", NULL, NULL);
  }
  }
  ...
  if (gasLogging) {
    ...
    if (gasLogCurrent!=gasLogLast) {
      ...
      if (gasLogCurrent==false){      
        mqttClient.publish(gasTopic, "Gas Pulse!");
...
void messageReceived(String topic, String payload, char * bytes, int length) {
...
// MQTT Callback routine to handle when a message arrives on subscribed topic
void mqttCallback(char *topic, byte *payload, int length) {
